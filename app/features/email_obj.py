...
class print_email:
    ...
    def search_email(self, search_string):
        results = self.service.users().messages().list(userId=self.user_id, maxResults=1, q=search_string).execute()  # this is where you can change the amount of emails to be printed --- maxResults=1 
        messages = results.get('messages', [])
        emails = []
        for message in messages:
            email_id = message['id']
